...
<main style="position:relative;top: 40px;">
    ...
    <p style="text-align: justify;">
        ...
        if($generoalumno=='Femenino'){
         ?>
         La alumna <strong>{{$nombrecompleto}}</strong> de <strong>{{$gradoalumno}}</strong> ha obtenido las siguientes calificaciones durante el año lectivo <strong>{{$descripcionaño}}</strong>. 
        <?php 
        }
        if($generoalumno=='Masculino'){
         ?>
         El alumno <strong>{{$nombrecompleto}}</strong> de <strong>{{$gradoalumno}}</strong> ha obtenido las siguientes calificaciones durante el año lectivo <strong>{{$descripcionaño}}</strong>. 
        <?php
        }
        ?>
    </p>
    ...
    ?>
        ...
        $cantidadperiodo=4;
        ?>
        <th class="tabla" style="width:15%;">Primer período</th>
        <th class="tabla" style="width:15%;">Segundo período</th>
        <th class="tabla" style="width:15%;">Tercer período</th>
        <th class="tabla" style="width:15%;">Cuarto período</th>
        @endif
        ...
        <th class="tabla" style="width:15%;">Segundo período</th>
        @endif
        <th class="tabla" style="width:12%;">Nota final</th>
        </thead>
        <tbody>
        <?php 
        ...
        if($informe->periodo=='Primer período'){
        $notasespacio[0]=$informe->nota;
        }
        elseif($informe->periodo=='Segundo período'){
        $notasespacio[1]=$informe->nota;
        }
        elseif($informe->periodo=='Tercer período'){
        $notasespacio[2]=$informe->nota;
        }
        elseif($informe->periodo=='Cuarto período'){
        $notasespacio[3]=$informe->nota;
        }
        elseif($informe->periodo=='Final'){
        $notasespacio[$cantidadperiodo]=$informe->nota;
        }        
        }
        ?>
        @endforeach
        ...
        elseif($informe->periodo=='Final'){
        $observacionesespacio[$cantidadperiodo]=$informe->observacion;
        }
        }
        ?> 
        @endforeach
        ...
        <table class="tabla" style="width: 100%;page-break-after:auto;" >
        <thead style="background-color:#BEC2DD;">
        <th class="tabla" style="width:30%;">Período</th>
        <th class="tabla" style="width:30%;">Observaciones</th>
        </thead>
        <tbody>
        @if($informacionperiodo=='Bimestre')
        <tr><td class="tabla">Primer período</td>
        <td class="tabla">{{$observacionesespacio[0]}}</td></tr>
        <tr><td class="tabla">Segundo período</td>
        <td class="tabla">{{$observacionesespacio[1]}}</td></tr>
        <tr><td class="tabla">Tercer período</td>
        <td class="tabla">{{$observacionesespacio[2]}}</td></tr>
        <tr><td class="tabla">Cuarto período</td>
        ...
        @if($informacionperiodo=='Cuatrimestre')
        <tr><td class="tabla">Primer período</td>
        <td class="tabla">{{$informe->observacion[0]}}</td></tr>
        <tr><td class="tabla">Segundo período</td>
        <td class="tabla">{{$informe->observacion[1]}}</td></tr>
        @endif
        @if($informacionperiodo=='Semestre')
        <tr><td class="tabla">Primer período</td>
        <td class="tabla">{{$informe->observacion[0]}}</td></tr>
        ...
        <br>
        {{$observacionesespacio[$cantidadperiodo]}}</div>
        <style>
        .cuadrado{
        padding:5px;
        width: 98.5%;
